...
public class AutonymActivity extends AppCompatActivity implements View.OnClickListener, HttpCallBack {
    ...
    private Button btnPromptlyAutonym;
    ...
    private void initView() {
        ...
        mHttpCallBack = this;
        tvTisji = findViewById(R.id.tv_tisji);
    }
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    jsonObject.put("idNo", etIdnum.getText().toString().trim());
                    Log.e("IdImages", img);
        ...
        switch (requestId) {
            ...
            case 3:
                try {
                    ...
                    if (mIdCard.isSuc()) {
                        //实名认证成功
                        tvTisji.setVisibility(View.VISIBLE);
                        Toast.makeText(this, "上传信息成功", Toast.LENGTH_SHORT).show();
                        finish();
                    } else {
                        //实名认证失败
                        Toast.makeText(this, "数据有误请重新上传", Toast.LENGTH_SHORT).show();
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                }
