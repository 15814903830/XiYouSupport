 ...
 * @Description:
 */
public class AccompanyUserInfoAdapter extends BaseRVAdapter<AccompanyUserInfoEntity.AccompanyPlayBean, BaseViewHolder> {
    public AccompanyUserInfoAdapter(int layoutResId, @Nullable List<AccompanyUserInfoEntity.AccompanyPlayBean> data) {
        super(layoutResId, data);
    }

    @Override
    protected void convert(BaseViewHolder helper, AccompanyUserInfoEntity.AccompanyPlayBean item) {
        ImageLoaderManager.getInstance().showImage(helper.getView(R.id.search_game_iv), APIContents.HOST + "/" + item.getImages());
        helper.setText(R.id.search_game_type_tv, item.getSubject().getTitle());
