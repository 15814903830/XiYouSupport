 ...
 * @Description:
 */
public class MineOngoingModelImpl implements MineOngoingContract.Model {
    @Override
    public Observable<List<MineOrderPlaceEntity>> PLACE_ONGOING_OBSERVABLE(int page) {
        return HttpRequest.get(APIContents.MyCreateOrder)
                .params(APPContents.E_ID, AppData.getString(AppData.Keys.AD_USER_ID))
                .params(APPContents.E_ORDER_STATUS, 2)
    ...
    @Override
    public Observable<List<MineOrderTakingEntity>> TAKING_ONGOING_OBSERVABLE(int page) {
