 ...
 * @Description:
 */
public class MineCompleteModelImpl implements MineCompleteContract.Model {
    @Override
    public Observable<List<MineOrderPlaceEntity>> PLACE_COMPLETE_OBSERVABLE(int page) {
        return HttpRequest.get(APIContents.MyCreateOrder)
                .params(APPContents.E_ID, AppData.getString(AppData.Keys.AD_USER_ID))
                .params(APPContents.E_ORDER_STATUS, 4)
    ...
    @Override
    public Observable<List<MineOrderTakingEntity>> TAKING_COMPLETE_OBSERVABLE(int page) {
