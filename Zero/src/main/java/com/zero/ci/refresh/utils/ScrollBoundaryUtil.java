...
public class ScrollBoundaryUtil {
    ...
    public static boolean canRefresh(@NonNull View targetView, PointF touch) {
        if (isCanScrollUp(targetView) && targetView.getVisibility() == View.VISIBLE) {
            return false;
        }
    ...
    public static boolean isCanScrollUp(@NonNull View targetView) {
...
//        final boolean isInView = pointInView(child, point[0], point[1], 0);
    ...
    public static boolean canLoadMore(@NonNull View targetView, PointF touch, boolean contentFull) {
        if (isCanScrollDown(targetView) && targetView.getVisibility() == View.VISIBLE) {
            return false;
        }
        ...
        return (contentFull || isCanScrollUp(targetView));
    }

    public static boolean isCanScrollDown(@NonNull View targetView) {
        if (android.os.Build.VERSION.SDK_INT < 14) {
            if (targetView instanceof AbsListView) {
                final ViewGroup viewGroup = (ViewGroup) targetView;
                final AbsListView absListView = (AbsListView) targetView;
