package com.zero.ci.refresh.api.constant;

/**
 * Author: Zero Yuan
 * Email: zero.yuan.xin@gmail.com
 * Description: 尺寸值的定义状态，用于在值覆盖的时候决定优先级
 * 越往下优先级越高
 * -------------------------------
 */

public enum DimensionStatus {
    DefaultUnNotify(false),//默认值
    Default(true),//默认值
    XmlWrapUnNotify(false),//Xml计算
    XmlWrap(true),//Xml计算
    XmlExactUnNotify(false),//Xml 的view 指定
    XmlExact(true),//Xml 的view 指定
    XmlLayoutUnNotify(false),//Xml 的layout 中指定
    XmlLayout(true),//Xml 的layout 中指定
    CodeExactUnNotify(false),//代码指定
    CodeExact(true),//代码指定
    DeadLockUnNotify(false),//锁死
    DeadLock(true);//锁死
