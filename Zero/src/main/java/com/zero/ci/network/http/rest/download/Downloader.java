...
import com.zero.ci.network.http.error.ServerError;
import com.zero.ci.network.http.error.StorageReadWriteError;
import com.zero.ci.network.http.error.StorageSpaceNotEnoughError;
    ...
    private void validateDevice(String savePathDir) throws Exception {
        ...
        if (!IOUtils.createFolder(savePathDir))
            throw new StorageReadWriteError("SD card isn't available, please check SD card and permission: WRITE_EXTERNAL_STORAGE." +
                    "\nYou must pay attention to Android6.0 RunTime Permissions:  AndPermission." +
        ...
        try {
            ...
            if (responseCode >= 400) {
                ...
                if (request.isCanceled()) {
                    Logger.w("NoHttpDownloader", "Download handle is canceled.");
                ...
                while (((len = serverStream.read(buffer)) != -1)) {
                    if (request.isCanceled()) {
                        Logger.i("NoHttpDownloader", "Download handle is canceled.");
                        downloadListener.onCancel(what);
                        break;
                    } else {
            ...
            Logger.e(newException.getMessage());
